...
const CACHE_VERSION = 'yarz-v7.0.0';
...
// ✅ v9.7: Message listener — admin panel can trigger cache clear after saving settings.
// Usage from page: navigator.serviceWorker.controller.postMessage({ type: 'CLEAR_API_CACHE' })
self.addEventListener('message', function (event) {
  if (event.data && event.data.type === 'CLEAR_API_CACHE') {
    caches.keys().then(function (keys) {
      return Promise.all(keys.filter(function(k) {
        return k.indexOf('-api') !== -1;
...
self.addEventListener('fetch', function (event) {
  ...
  // CSS / JS → network-first with fast timeout (always latest code, offline fallback)
  if (req.destination === 'style' || req.destination === 'script') {
    event.respondWith(networkFirst(req, STATIC_CACHE, 2000));
    return;
  }
