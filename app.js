...
const YARZ = (() => {
  ...
  // ===== IMAGE URL PROCESSOR (Same logic as Admin Panel) =====
  // Auto-fixes URLs: adds https://, converts Google Drive, ibb.co short links etc.
  function getImgSrc(url) {
    ...
    // Convert ibb.co short links to direct image
    if (url.match(/^https?:\/\/(www\.)?ibb\.(co|co\.com)\/[a-zA-Z0-9]+$/)) {
      url = url.replace(/ibb\.(co|co\.com)\//, 'i.ibb.co/') + '/image.jpg';
    }
    return url;
  }
  ...
  function openProduct(name) {
    ...
    var sizes = ['M', 'L', 'XL', 'XXL'];
    var deliveryDhaka = parseFloat(product.deliveryDhaka) || (state.storeInfo && state.storeInfo.zone1Charge !== undefined ? state.storeInfo.zone1Charge : 60);
    var deliveryOutside = parseFloat(product.deliveryOutside) || (state.storeInfo && state.storeInfo.zone2Charge !== undefined ? state.storeInfo.zone2Charge : 120);
    ...
    var z1 = state.storeInfo && state.storeInfo.zone1Name ? state.storeInfo.zone1Name : 'Dhaka (Inside)';
    var z2 = state.storeInfo && state.storeInfo.zone2Name ? state.storeInfo.zone2Name : 'Outside Dhaka';

    // Delivery info
    html += '<div class="pd-delivery-info">';
    html += '<div class="pd-delivery-row">' + ICONS.truck + '<span>' + escHtml(z1) + ': ' + formatPrice(deliveryDhaka) + ' &middot; ' + escHtml(z2) + ': ' + formatPrice(deliveryOutside) + '</span></div>';
  ...
  function addToCart(product, size, qty) {
    ...
    if (existing) {
      ...
      state.cart.push({
        ...
        image: p.image1 || '',
        category: p.category || '',
        deliveryDhaka: parseFloat(p.deliveryDhaka) || (state.storeInfo && state.storeInfo.zone1Charge !== undefined ? state.storeInfo.zone1Charge : 60),
        deliveryOutside: parseFloat(p.deliveryOutside) || (state.storeInfo && state.storeInfo.zone2Charge !== undefined ? state.storeInfo.zone2Charge : 120),
  ...
  function openCheckout() {
    ...
    // Dynamically render location options based on admin settings
    var locationSel = $('#co-location');
    if (locationSel) {
      var z1 = state.storeInfo && state.storeInfo.zone1Name ? state.storeInfo.zone1Name : 'Dhaka (Inside)';
      var z2 = state.storeInfo && state.storeInfo.zone2Name ? state.storeInfo.zone2Name : 'Outside Dhaka';
      var currentLoc = locationSel.value;
      locationSel.innerHTML = '<option value="Dhaka">' + escHtml(z1) + '</option><option value="Outside">' + escHtml(z2) + '</option>';
      if (currentLoc) locationSel.value = currentLoc;
    }
    ...
    if (paymentSel) {
      ...
      if (!paymentSel.value) {
        paymentSel.value = 'COD';
      } else {
        paymentSel.value = currentVal || 'COD';
      }
    }
  ...
  function renderCheckoutSummary() {
    ...
    var location = ($('#co-location') || {}).value || 'Dhaka';
    var deliveryCharge = 0;
    if (state.cart.length > 0) {
      deliveryCharge = location === 'Dhaka' ? state.cart[0].deliveryDhaka : state.cart[0].deliveryOutside;
      state.cart.forEach(function(item) {
        var charge = location === 'Dhaka' ? item.deliveryDhaka : item.deliveryOutside;
  ...
  function submitOrder() {
    ...
    var enableCOD = state.storeInfo && state.storeInfo.enable_cod !== 'false';
    if (payment === 'COD' && !enableCOD) {
      showToast('অনুগ্রহ করে bKash বা Nagad সিলেক্ট করে অগ্রিম ডেলিভারি চার্জ প্রদান করুন।', 'warning');
      return;
    }
  ...
  function processOrderSubmission(name, phone, email, address, location, city, payment, trxid, orderSig) {
    ...
    var finalLocationName = location === 'Dhaka'
      ? (state.storeInfo && state.storeInfo.zone1Name ? state.storeInfo.zone1Name : 'Dhaka')
      : (state.storeInfo && state.storeInfo.zone2Name ? state.storeInfo.zone2Name : 'Outside Dhaka');

    // Build order promises
    var orderPromises = state.cart.map(function (item) {
      var deliveryCharge = location === 'Dhaka' ? item.deliveryDhaka : item.deliveryOutside;
    ...
    Promise.all(orderPromises).then(function (results) {
      ...
      try {
        ...
        var newLocalOrders = state.cart.map(function(item) {
          var deliveryCharge = location === 'Dhaka' ? item.deliveryDhaka : item.deliveryOutside;
          var itemPrice = item.price;
          if (state.appliedCoupon && item.couponActive === 'Yes' && (item.couponCode || '').toUpperCase() === state.appliedCoupon.code) {
      ...
      var fallbackTotal = state.cart.reduce(function(sum, item) {
        var deliveryCharge = location === 'Dhaka' ? item.deliveryDhaka : item.deliveryOutside;
        return sum + (item.price * item.qty) + deliveryCharge;
      }, 0);
  ...
  function openTracking() {
    ...
    var html = '<div class="tracking-section">' +
      ...
      '<p style="font-size:12px;color:var(--text-muted);font-family:var(--font-bengali);margin-top:4px;">আপনার ফোন নম্বর দিয়ে অর্ডার খুঁজুন</p></div>' +
  ...
  function searchOrders() {
    ...
    if (!phone || phone.length < 10) { showToast('Enter valid phone number', 'warning'); return; }
  ...
  // ===== IN-APP BROWSER DETECTOR (For iOS Facebook/Instagram) =====
  function initInAppBrowserWarning() {
    var ua = navigator.userAgent || navigator.vendor || window.opera;
    var isIOS = /iPad|iPhone|iPod/.test(ua) && !window.MSStream;
    var isInApp = (ua.indexOf('FBAV') > -1 || ua.indexOf('FBIOS') > -1 || ua.indexOf('Instagram') > -1);

    if (isIOS && isInApp) {
      if (document.getElementById('in-app-warning')) return;
      
      // Prevent scrolling
      document.body.style.overflow = 'hidden';

      var warning = document.createElement('div');
      warning.id = 'in-app-warning';
      warning.innerHTML = 
        // Transparent overlay to block clicks
        '<div style="position:fixed; top:0; left:0; width:100%; height:100vh; background:rgba(0,0,0,0.4); z-index:999998;"></div>' +
        
        // iOS Native Style System Notification
        '<div style="position:fixed; top:16px; left:16px; right:16px; background:rgba(255,255,255,0.85); backdrop-filter:blur(24px); -webkit-backdrop-filter:blur(24px); z-index:999999; padding:16px; border-radius:20px; box-shadow:0 8px 32px rgba(0,0,0,0.15); transform:translateY(-150%); transition:transform 0.6s cubic-bezier(0.16, 1, 0.3, 1); border:1px solid rgba(255,255,255,0.4); display:flex; align-items:center; gap:16px;">' +
          '<div style="width:52px; height:52px; flex-shrink:0; border-radius:12px; overflow:hidden; box-shadow:0 2px 8px rgba(0,0,0,0.1);">' +
            // Exact Safari Logo SVG Replica
            '<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" style="width:100%; height:100%;">' +
              ...
              '<g transform="rotate(45 50 50)">' +
                '<polygon points="46,18 54,18 50,50" fill="#E53935"/>' +
                '<polygon points="46,82 54,82 50,50" fill="#FFFFFF"/>' +
              '</g>' +
              '<circle cx="50" cy="50" r="3.5" fill="#FFFFFF"/>' +
            '</svg>' +
          '</div>' +
          '<div style="flex-grow:1;">' +
            '<h3 style="font-family:-apple-system, BlinkMacSystemFont, \'SF Pro Text\', \'Helvetica Neue\', sans-serif; font-size:16px; font-weight:600; color:#000000; margin:0 0 4px; letter-spacing:-0.4px;">Open in Safari</h3>' +
            '<p style="font-family:-apple-system, BlinkMacSystemFont, \'SF Pro Text\', \'Helvetica Neue\', sans-serif; font-size:14px; color:#3C3C43; margin:0; line-height:1.3; letter-spacing:-0.1px;">Please tap the <strong style="color:#000000;">⋯</strong> menu at the top right and select <strong>Open in System Browser</strong>.</p>' +
          '</div>' +
        '</div>';
      ...
      // Slide down smoothly
      setTimeout(function() {
        warning.lastChild.style.transform = 'translateY(0)';
      }, 300);
    }
  }
    ...
    Promise.all([
      ...
      if (productsRes.success && productsRes.products) {
        ...
        if (state.storeInfo && Object.keys(state.storeInfo).length > 0) {
          ...
          YARZ_API.getGlobalControls().then(function(c) {
            renderDynamicSections(state.products, c ? c.raw : {});
          });
        }
        renderProducts(state.products);
  ...
  function renderSocialLinks(links) {
    ...
    // Facebook
    if (links.facebook) {
      html += '<a href="' + escHtml(links.facebook) + '" target="_blank" rel="noopener" aria-label="Facebook">' +
              '<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg></a>';
    }
    // Instagram
    if (links.instagram) {
      html += '<a href="' + escHtml(links.instagram) + '" target="_blank" rel="noopener" aria-label="Instagram">' +
    ...
    // WhatsApp
    if (links.whatsapp) {
      html += '<a href="' + escHtml(links.whatsapp) + '" target="_blank" rel="noopener" aria-label="WhatsApp">' +
    ...
    // TikTok
    if (links.tiktok) {
      html += '<a href="' + escHtml(links.tiktok) + '" target="_blank" rel="noopener" aria-label="TikTok">' +
              '<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z"/></svg></a>';
    }
    // Messenger
    if (links.messenger) {
      html += '<a href="' + escHtml(links.messenger) + '" target="_blank" rel="noopener" aria-label="Messenger">' +
              '<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.477 2 2 6.145 2 11.258c0 2.915 1.487 5.503 3.791 7.21v3.532c0 .351.378.566.685.391l3.411-1.87c.683.188 1.393.287 2.113.287 5.523 0 10-4.145 10-9.258S17.523 2 12 2zm1.092 12.44l-2.451-2.617-4.78 2.617 5.253-5.56 2.451 2.618 4.78-2.618-5.253 5.56z"/></svg></a>';
    }
