...
const YARZ_TURBO = (() => {
  ...
  // ===== CONFIG =====
  const CFG = {
    CHUNK_SIZE: 8,                // Render first 8 products instantly
  ...
  function _patchFetch() {
    window.fetch = function(url, options) {
      // Only intercept Google APIs and Apps Script
      var urlStr = typeof url === 'string' ? url : (url && url.url ? url.url : '');
      if (urlStr.indexOf('googleapis.com') > -1 || urlStr.indexOf('script.google.com') > -1) {
  ...
  // Preload product images when user touches a card (before click)
  function _initTouchPrefetch() {
    document.addEventListener('touchstart', function(e) {
      ...
      // Prefetch high-res version
      var src = img.getAttribute('src') || img.src;
      if (src && !_prefetchedImages.has(src)) {
        _prefetchedImages.add(src);
