...
(function (global) {
  ...
  function optimize(url, size) {
    if (!url) return url;
    size = size || 1200;
    // Account for HiDPI / Retina screens — but cap DPR at 2 (3x is overkill, 2x looks sharp)
    var dpr = Math.min((window.devicePixelRatio || 1), 2);
    var realSize = Math.round(size * dpr);
    // Cap so we don't request absurd sizes
