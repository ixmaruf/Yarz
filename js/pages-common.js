...
(function () {
  ...
  // ---------- 1) FLOATING MESSENGER BUTTON (v4.3: Smart Deep Link) ----------
  function injectFloatingMessenger(messengerUrl) {
    ...
    if (document.getElementById('floating-messenger-btn')) {
      ...
      _attachMsgrDeepLink(existing, messengerUrl);
      return;
    }
    ...
    _attachMsgrDeepLink(btn, messengerUrl);
  }

  // v4.3: Smart Messenger Deep Link — opens app directly on mobile
  function _attachMsgrDeepLink(btn, msUrl) {
    if (btn._yarzDeepLink) return;
    btn._yarzDeepLink = true;
    btn.addEventListener('click', function(e) {
      ...
      if (isAndroid) {
        var t = setTimeout(function(){ window.location.href = msUrl; }, 1500);
        window.location.href = 'fb-messenger://user-thread/' + pageId;
        window.addEventListener('blur', function f(){ clearTimeout(t); window.removeEventListener('blur', f); }, {once:true});
      } else {
        var t2 = setTimeout(function(){ window.location.href = msUrl; }, 1200);
        window.location.href = 'fb-messenger://user-thread/' + pageId;
        document.addEventListener('visibilitychange', function f(){ if(document.hidden) clearTimeout(t2); document.removeEventListener('visibilitychange', f); }, {once:true});
      }
    });
  }
