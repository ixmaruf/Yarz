...
const YARZ_SHIELD = (() => {
  ...
  function validate(orderData) {
    ...
    // 2. Click Cooldown (rapid clicking)
    var now = Date.now();
    if (now - _lastOrderClickTime < CFG.CLICK_COOLDOWN_MS) {
      return { allowed: false, reason: 'অনুগ্রহ করে কিছুক্ষণ অপেক্ষা করুন।', silent: false };
    }
    ...
    // 5. Phone rate limit: max 10 orders per hour
    var phoneOrdersHour = _countOrders(
      function(e) { return e.phone === phone; },
      60 * 60 * 1000
    );
    if (phoneOrdersHour >= CFG.MAX_ORDERS_PER_HOUR) {
      return { allowed: false, reason: 'এই নম্বর দিয়ে আপাতত আর অর্ডার করা যাচ্ছে না। কিছুক্ষণ পর আবার চেষ্টা করুন।', silent: false };
    }

    // 6. Phone rate limit: max 15 orders per 24 hours
    var phoneOrdersDay = _countOrders(
      function(e) { return e.phone === phone; },
      24 * 60 * 60 * 1000
    );
    if (phoneOrdersDay >= CFG.MAX_ORDERS_PER_DAY) {
      return { allowed: false, reason: 'আজকের জন্য এই নম্বরের সর্বোচ্চ অর্ডার সীমা পূর্ণ হয়েছে। আগামীকাল আবার চেষ্টা করুন।', silent: false };
    }

    // 7. Device rate limit: max 15 orders per 24 hours
    var deviceId = _getDeviceId();
    var deviceOrdersDay = _countOrders(
      function(e) { return e.deviceId === deviceId; },
      24 * 60 * 60 * 1000
    );
    if (deviceOrdersDay >= CFG.MAX_ORDERS_PER_DAY) {
      return { allowed: false, reason: 'এই ডিভাইস থেকে আজকের জন্য সর্বোচ্চ অর্ডার দেওয়া হয়েছে।', silent: false };
    }
