...
var YARZ_API = (function() {
  ...
  // ✅ SECURITY: Google API key and Sheet ID removed — Supabase/Worker is primary
    ...
    if (action === 'products') {
      ...
      if (Array.isArray(data.products)) {
        data.products = data.products.map(function(p) {
          ...
          if (!p.image5 && p.img5) p.image5 = p.img5;
          if (!p.image6 && p.img6) p.image6 = p.img6;
          // ✅ Worker snake_case image fields → camelCase
          if (!p.image1 && p.image_1) p.image1 = p.image_1;
          if (!p.image2 && p.image_2) p.image2 = p.image_2;
          if (!p.image3 && p.image_3) p.image3 = p.image_3;
          if (!p.image4 && p.image_4) p.image4 = p.image_4;
    ...
    try {
      ...
      for (let i = 1; i <= 5; i++) {
        ...
        if (img) {
          heroBanners.push({
            image: img,
            title: (s['banner_title_' + i] || s['banner_title ' + i] || '').replace(/YARZ[\s\u2014\u2013\u2012\-]+Premium\s*Men[\u0027\u2019\u2018]?\s*s\s*Fashion/gi, '').trim(),
