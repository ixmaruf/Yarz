...
const YARZ_API = (() => {
  ...
  const APPS_SCRIPT_URL = _0x('aHR0cHM6Ly9zY3JpcHQuZ29vZ2xlLmNvbS9tYWNyb3Mvcy9BS2Z5Y2J6aDlkNzFNbXRNX3MxUFM2ZF9zdndmbzYwNXhVUjFYQXZST2FldTU2SDZfd0VXNVpVLXVnYmZGTjUwb25DU3FqdWkvZXhlYw==');
  ...
  const DEPLOY_VERSION = '2026-05-17-v9.8-final-deploy';
  ...
  const CONFIG = {
    ...
    SETTINGS_CACHE_TTL: 10 * 1000,            // 10 seconds fresh (v9.7: reduced from 30s for faster admin sync)
    SETTINGS_STALE_TTL: 30 * 1000,            // 30 seconds stale (v9.7: reduced from 2min)
  };
  ...
  async function getGlobalControls() {
    try {
      ...
      const get = (key) => {
        // ✅ v9.7 FIX: Check multiple key formats to handle differences between
        // what admin panel saves (e.g. 'Enable COD') and how backend normalizes
        // (e.g. 'enable_cod'). Also check the original mixed-case key directly.
        const normalized = key.toLowerCase().replace(/[\s()]+/g, '_');
        if (s[normalized] !== undefined) return s[normalized];
        // Check original key (admin panel saves mixed-case like 'Enable COD')
        if (s[key] !== undefined) return s[key];
        // Check Title Case variant (e.g. 'enable_cod' → 'Enable COD')
      ...
      for (let i = 1; i <= 50; i++) {
        const title = String(get(`section_${i}_title`) || '');
        const active = parseBool(get(`section_${i}_active`), true);
        if (title && active) {
          const rawLink = String(get(`section_${i}_link`) || '');
          let links = [];
          try {
    ...
    try {
      ...
      try {
        ...
        if (snapshot) {
          ...
          if (snap && snap.time && (Date.now() - snap.time) < 10 * 60 * 1000) { // 10min max
