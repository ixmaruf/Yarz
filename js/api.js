...
const YARZ_API = (() => {
  ...
  // ✅ v3.9: PREFETCH — Fire products + store_info in parallel as soon as api.js loads.
  // Result: by the time app.js renders, data is already in cache → instant first paint.
  ...
  // Auto-prefetch on script load (parallel with HTML/CSS parsing)
  if (typeof window !== 'undefined') {
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', prefetchAll, { once: true });
    } else {
      prefetchAll();
    }
  }
