/* YARZ API v6.0 — Cloudflare Worker Edge Cache integration */
...
const YARZ_API = (() => {
  ...
  // ⚡ NEW v6.0: Cloudflare Worker reverse-proxy URL.
  // Customers hit this instead of script.google.com — sub-100ms response,
  // 14-min edge cache, webhook-driven invalidation.
  const CLOUDFLARE_WORKER_URL = 'https://yarz.marufhasan80009.workers.dev/';

  // Original Google Apps Script URL — kept as ultimate fallback
  // in case the Worker is ever unreachable.
  const APPS_SCRIPT_URL = _0x('aHR0cHM6Ly9zY3JpcHQuZ29vZ2xlLmNvbS9tYWNyb3Mvcy9BS2Z5Y2J3YWNTLUpKZG56R0d6WElvcl83OUdtWmJSVzBfWHk4alNYS21TLTZOWnJLVTZQZXFjZk5YdFVjWldzcDdab2lqVDUvZXhlYw==');
  ...
  var _turboPromise = (function _turboPreload() {
    try {
      // ⚡ v10.7: Reuse the super-early fetch from index.html if available
      // (started during HTML parse, ~100-300ms before this script even runs)
      var earlyFetch = window.__YARZ_EARLY_FETCH;
      var fetchPromise;
      if (earlyFetch && typeof earlyFetch.then === 'function') {
        // Race the early fetch against a 4s safety timeout — if Edge's first
        // request is slow, fire a fresh one rather than blocking forever
        fetchPromise = Promise.race([
          earlyFetch,
          new Promise(function(resolve) {
            setTimeout(function() { resolve('__TIMEOUT__'); }, 4000);
          })
        ]).then(function(result) {
          if (result === '__TIMEOUT__' || !result) {
            // Early fetch timed out or failed — fire a fresh one
            var url = CLOUDFLARE_WORKER_URL + '?key=' + GOOGLE_API_KEY + '&action=products';
        ...
        var url = CLOUDFLARE_WORKER_URL + '?key=' + GOOGLE_API_KEY + '&action=products';
        fetchPromise = fetch(url).then(function(r) { return r.json(); });
      }
      return fetchPromise
        .then(function(json) {
  ...
  const DEPLOY_VERSION = '2026-05-21-v10.10-long-cache-webp';
  ...
  const CONFIG = {
    ...
    CACHE_TTL: 5 * 1000,
    STALE_TTL: 20 * 1000,
    PRODUCT_CACHE_TTL: 30 * 1000,             // 30 seconds fresh (CF Worker has 60s edge cache)
    PRODUCT_STALE_TTL: 60 * 1000,             // 60 seconds stale (serve old while revalidating)
    SETTINGS_CACHE_TTL: 30 * 1000,            // 30 seconds fresh
    SETTINGS_STALE_TTL: 60 * 1000,            // 60 seconds stale
  };
  ...
  const API_ENDPOINTS = [
    'https://script.google.com/macros/s/AKfycbwacS-JJdnzGGzXIor_79GmZbRW0_Xy8jSXKmS-6NZrKU6PeqcfNXtUcZWsp7ZoijT5/exec'
  ];
  ...
  async function apiPost(action, body = {}) {
    // ⚡ v6.0: Writes go direct to Apps Script (Worker also forwards POST,
    // but order placement doesn't need an extra hop). Override via
    // localStorage.yarz_post_url if you want to route through the Worker.
    const base = localStorage.getItem('yarz_post_url') || getWriteUrl();
    if (!base) throw new Error('API URL not configured');
