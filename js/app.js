...
const YARZ = (() => {
  ...
  function goHome(e) {
    ...
    try {
      ...
      try {
        document.title = state._originalTitle || 'YARZ — Premium Men\'s Fashion | পুরুষ ফ্যাশন';
        var metaD = document.querySelector('meta[name="description"]');
        if (metaD) metaD.content = state._originalDesc || 'YARZ — বাংলাদেশের প্রিমিয়াম পুরুষ ফ্যাশন ব্র্যান্ড।';
  ...
  function renderDynamicSections(products, storeInfo) {
    ...
    // ✅ v9.7 FIX: Prefer pre-parsed dynamicSections from getGlobalControls() if available.
    // These are already correctly parsed from multiple key formats. Raw key lookup is fallback.
    if (storeInfo._parsedDynamicSections && storeInfo._parsedDynamicSections.length > 0) {
      sections = storeInfo._parsedDynamicSections;
    } else {
      // Fallback: raw key lookup with multi-format checking
      for (var i = 1; i <= 50; i++) {
        var title = storeInfo['section_' + i + '_title'] || storeInfo['section_' + i + 'title']
                 || storeInfo['Section ' + i + ' Title'] || '';
        var category = storeInfo['section_' + i + '_category'] || storeInfo['section_' + i + 'category']
                    || storeInfo['Section ' + i + ' Category'] || '';
        var rawLink = storeInfo['section_' + i + '_link'] || storeInfo['section_' + i + 'link']
                   || storeInfo['Section ' + i + ' Link'] || '';
        var image = storeInfo['section_' + i + '_image'] || storeInfo['section_' + i + 'image']
                 || storeInfo['Section ' + i + ' Image'] || '';
        var linkArray = [];
        if (rawLink) {
          try { linkArray = JSON.parse(rawLink); if(!Array.isArray(linkArray)) linkArray = [rawLink]; }
          catch(e) { linkArray = [rawLink]; }
        }
        if (title) sections.push({ title: title, category: category, links: linkArray, image: image });
      }
    }
    ...
    sections.forEach(function (sec, idx) {
      ...
      html += '</div>';
      // v9.8: Section banner image support
      if (sec.image) {
        html += '<div class="dynamic-section-banner">';
        if (sec.link) html += '<a href="' + escHtml(sec.link) + '">';
        html += '<img src="' + escHtml(getImgSrc(sec.image)) + '" alt="' + escHtml(sec.title) + '" loading="lazy" onerror="this.parentElement.style.display=\'none\'">';
        if (sec.link) html += '</a>';
        html += '</div>';
      }
  ...
  function openProduct(name) {
    ...
    if (product.couponActive === 'Yes' && product.couponCode && parseFloat(product.couponDisc) > 0) {
      html += '<div class="coupon-card" onclick="navigator.clipboard.writeText(\'' + escHtml(product.couponCode) + '\');YARZ.showToast(\'কুপন কোড কপি করা হয়েছে! ✅\',\'success\');">' +
  ...
  function init() {
    ...
    YARZ_API.getGlobalControls().then(function (controls) {
      ...
      // ── Promotional Popup (v9.8: CSS-class design) ──
      if (controls.promoPopupActive && controls.promoPopupImage) {
        ...
        if (!sessionStorage.getItem(dismissedKey)) {
          setTimeout(function() {
            ...
            var popupHtml = '<div class="yarz-promo-popup-card">';
            popupHtml += '<button class="popup-close" onclick="var o=document.getElementById(\'yarz-promo-popup\');if(o)o.remove();sessionStorage.setItem(\'' + dismissedKey + '\',\'1\')">✕</button>';
            if (controls.promoPopupLink) {
              popupHtml += '<a href="' + escHtml(controls.promoPopupLink) + '" style="display:block;line-height:0;">';
            }
            popupHtml += '<img src="' + escHtml(getImgSrc(controls.promoPopupImage)) + '" alt="Promo" onerror="var o=document.getElementById(\'yarz-promo-popup\');if(o)o.remove()">';
