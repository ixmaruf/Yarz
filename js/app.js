...
const YARZ = (() => {
  ...
  function renderProductCard(p, index) {
    ...
    // ✅ v11.8: Quick View button — admin-controlled
    if (state.controls && state.controls.quickViewActive) {
  ...
  function applyExtrasControls(controls) {
    ...
    // ✅ v15.2 — Legacy lavender purple guard.
    // The previous v14 default theme palette was lavender purple. After
    // the v15 burgundy heritage redesign, any saved legacy purple value
    // in the Sheet would override and break the new theme. Skip those
    // specific legacy values so the burgundy CSS default wins. Admin can
    // still set ANY other custom color (red/green/gold/blue/etc.).
    var legacyPurples = [
      '#634A8E', '#4E3A72', '#634A8E', '#2A1E3E',
      '#7B5EA7', '#9B7DBF'
    ];
    function isLegacyPurple(v) {
      return v && legacyPurples.indexOf(String(v).trim().toLowerCase()) !== -1;
    }

    // 1. Theme Palette overrides — v11.3 expanded
    if (controls.themePrimary && !isLegacyPurple(controls.themePrimary)) {
      ...
      // ✅ v14.5: REMOVED "meta.content = themePrimary" — that was painting the
      // browser address bar PURPLE while the header is CREAM, creating a
      // visible seam. Theme-color is now driven by the header/announcement
      // bg via syncBrowserChromeColor() below — auto-tracks scroll state.
    }
    if (controls.themeAccent && !isLegacyPurple(controls.themeAccent)) {
    ...
    if (controls.themeLink && !isLegacyPurple(controls.themeLink)) {
      root.style.setProperty('--link-color', controls.themeLink);
    }
    if (controls.themeSalePrice) {
      // ✅ v14.3: Legacy red (#C0392B / #E53935) setting was the old default —
      // we've moved to premium black for readability. If the saved value is
      // one of those legacy reds, skip the override so the new black default
      // wins. Admin can still set ANY other custom color.
      var legacyReds = ['#c0392b', '#e53935', '#dc2626', '#b91c1c', '#ef4444'];
      var savedColor = String(controls.themeSalePrice).trim().toLowerCase();
      if (legacyReds.indexOf(savedColor) === -1) {
  ...
  function openProduct(name) {
    ...
    html += '<div class="pd-main-image" id="pd-main-img"><img src="' + escHtml(instantThumbUrl) + '" data-src="' + escHtml(rawMainUrl) + '" data-size="1600" alt="' + escHtml(product.name) + '" id="pd-img-main" fetchpriority="high" decoding="sync" style="transition: filter 0.3s ease; filter: blur(0px);"></div>';
    ...
    if (images.length > 1) {
      ...
      images.forEach(function (img, i) {
        ...
        html += '<div class="pd-thumb' + (i === 0 ? ' active' : '') + '" onclick="YARZ.switchImage(' + i + ',\'' + escHtml(getImgSrc(img)).replace(/'/g, "\\'") + '\')"><img src="' + escHtml(getImgSrc(img)) + '" alt="' + escHtml(product.name) + ' thumbnail" data-size="200" loading="lazy" decoding="async"></div>';
      });
      html += '</div>';
    }
    html += '</div>';
    ...
    if (state.controls && state.controls.trustStripActive && state.controls.trustBadges && state.controls.trustBadges.length) {
      html += '<div class="pd-trust-strip">';
      state.controls.trustBadges.forEach(function(b) {
    ...
    // Related Products section - ALWAYS show as requested
    try {
  ...
  function submitOrder() {
    ...
    // ✅ v11.6 FIX: Keep notes/custom/trxid OUT of the address line.
    // They get appended to a separate `notes` field below so courier slip
    // shows only a clean delivery address, not bracketed metadata.
    var orderNotes = '';
    ...
    if (customFieldEl && customFieldEl.value.trim()) {
      customFieldValue = (state.customField || 'Custom') + ': ' + customFieldEl.value.trim();
    }
  ...
  function renderHeroBannersFromStore(store) {
    ...
    if (banners.length > 0) {
      ...
      try {
        var firstBannerUrl = getImgSrc(banners[0].image, 2000);
        if (firstBannerUrl) {
      ...
      if (slider && dotsContainer) {
        var slidesHtml = banners.map(function (b, i) {
          // v13.1: 2000px hint — sharp on retina laptops + 4K, still fast on mobile via SW cache
          var bannerSrc = getImgSrc(b.image, 2000);
          // v10.5 SUPER POWERFUL: Absolute highest priority for First Hero Banner
          var eagerTags = i === 0 ? 'fetchpriority="high" loading="eager" decoding="sync"' : 'loading="lazy" decoding="async"';
          ...
          var imgHtml = '<img src="' + escHtml(bannerSrc) + '" alt="' + escHtml(b.title) + '" ' + eagerTags + ' style="width:100%;height:100%;object-fit:cover;object-position:center center;display:block;" onerror="this.style.display=\'none\'">';
  ...
  function init() {
    ...
    YARZ_API.getGlobalControls().then(function (controls) {
      ...
      if (controls.themeColor) {
        // ✅ v15.2: Legacy lavender purple guard — skip pre-v15 default
        // values so burgundy CSS wins. Admin can still set any other color.
        var _legacyLavender = ['#634A8E','#4E3A72','#634A8E','#2A1E3E','#7B5EA7','#9B7DBF'];
        var _saved = String(controls.themeColor).trim().toLowerCase();
        if (_legacyLavender.indexOf(_saved) === -1) {
        ...
        // ✅ v14.5: REMOVED address-bar paint with accent color — it was
        // creating purple-vs-cream seam against the cream header. The
        // address bar now follows the actual header/announcement bg via
        // syncBrowserChromeColor() — keeps the seam invisible.
      }

      // ── Premium Full Theme Palette (v11.3 expanded) ──
      if (controls.themePrimary) document.documentElement.style.setProperty('--brand', controls.themePrimary);
      if (controls.themeAccent) {
        document.documentElement.style.setProperty('--accent', controls.themeAccent);
        document.documentElement.style.setProperty('--accent-hover', controls.themeAccent);
      }
      if (controls.themeBg) document.documentElement.style.setProperty('--bg-body', controls.themeBg);
      if (controls.themeCardBg) document.documentElement.style.setProperty('--bg-card', controls.themeCardBg);
      if (controls.themeText) document.documentElement.style.setProperty('--text-main', controls.themeText);
      if (controls.themeBorder) {
        document.documentElement.style.setProperty('--border-color', controls.themeBorder);
        document.documentElement.style.setProperty('--border-light', controls.themeBorder);
      }
      if (controls.themeLink) document.documentElement.style.setProperty('--link-color', controls.themeLink);
      if (controls.themeSalePrice) {
        document.documentElement.style.setProperty('--sale-price', controls.themeSalePrice);
        document.documentElement.style.setProperty('--price-color', controls.themeSalePrice);
      }
      if (controls.themeFooterBg) document.documentElement.style.setProperty('--footer-bg', controls.themeFooterBg);
