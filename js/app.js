...
const YARZ = (() => {
  ...
  function getImgSrc(url) {
    ...
    if (url.indexOf('drive.google.com') !== -1) {
      ...
      // =s0 → original size (no compression). Falls back to =w2000 if =s0 blocked.
  ...
  function getDeliveryLocations() {
    ...
    if (!locations.length) {
      var z1Name = info.zone1Name || info.zone_1_name || 'Dhaka (Inside)';
      var z2Name = info.zone2Name || info.zone_2_name || 'Outside Dhaka';
      var z1Charge = parseFloat(info.zone1Charge || info.zone_1_charge || 60) || 60;
      var z2Charge = parseFloat(info.zone2Charge || info.zone_2_charge || 120) || 120;
      locations = [
        { id: 'Dhaka', name: z1Name, charge: z1Charge, active: true },
        { id: 'Outside', name: z2Name, charge: z2Charge, active: true }
      ];
    }
    return locations;
  }
  ...
  function getDeliveryLocationName(locationId) {
    var loc = getDeliveryLocationById(locationId);
    return loc ? loc.name : 'Dhaka (Inside)';
  }
  ...
  function addToCart(product, size, qty) {
    ...
    if (existing) {
      ...
      state.cart.push({
        ...
        image: p.image1 || '',
        category: p.category || '',
        deliveryDhaka: parseFloat(p.deliveryDhaka) || getDeliveryCharge('Dhaka'),
        deliveryOutside: parseFloat(p.deliveryOutside) || getDeliveryCharge('Outside'),
  ...
  function renderCheckoutSummary() {
    ...
    var location = ($('#co-location') || {}).value || (getDeliveryLocations()[0] || {}).id || 'Dhaka';
