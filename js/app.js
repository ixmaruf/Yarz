...
const YARZ = (() => {
  ...
  // ✅ v4.1: Format any date input (ISO string, epoch ms, Date object, Sheet date)
  // into Bangladesh local time: "03 May 2026, 02:45 PM"
  function _fmtBdDate(input) {
    ...
    try {
      if (input instanceof Date) d = input;
        ...
        var s = input.trim();
        // Already a nice display string? Keep as-is
    ...
    window.addEventListener('popstate', function () {
      try {
        ...
        if (hash.indexOf('#product/') === 0) {
          ...
          if (productName && state.products && state.products.length > 0) {
            var p = state.products.find(function(pr){ return pr.name === productName; });
            if (p) { openProduct(productName); return; }
          }
        }
  ...
  function init() {
    ...
    (function _tryInstantRender() {
      try {
        ...
        var cachedProducts  = localStorage.getItem('yarz_api_cache_products');
        var cachedCategories = localStorage.getItem('yarz_categories_cache');
        if (cachedProducts) {
          var parsed = JSON.parse(cachedProducts);
          var age = Date.now() - (parsed.time || 0);
          // v4.3: Extended stale window to 4 hours — products rarely change that fast
          if (age < 4 * 60 * 60 * 1000 && parsed.data && parsed.data.products) {
            state.products = parsed.data.products;
            if (cachedStoreInfo) {
  ...
  function renderLiveChatButtons(liveChat, socialLinks) {
    ...
    if (staticBtn) {
      if (msUrl) {
        ...
        // v4.3: Attach smart deep link handler if not already attached
        if (!staticBtn._yarzDeepLinkAttached) {
          staticBtn._yarzDeepLinkAttached = true;
          _attachMessengerDeepLink(staticBtn, msUrl);
        }
      } else {
