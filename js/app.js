...
const YARZ = (() => {
  ...
  function _populateSizeDropdown(cat) {
    var sizeSel = document.getElementById('filter-size-select');
    var sizeList = document.getElementById('filter-size-list');
    if (!sizeSel) return;
    ...
    if (isPant) {
      ...
      customHtml += '<div class="yarz-custom-dropdown__optgroup-label">Upper Body</div>';
      ...
      customHtml += '<div class="yarz-custom-dropdown__optgroup-label">Lower Body</div>';
      var waist = [
        { code: 'S',   label: '28"' },
        { code: 'M',   label: '30"' },
        { code: 'L',   label: '32"' },
    ...
    if (sizeList) sizeList.innerHTML = customHtml;
    // Reset custom dropdown label to "All Sizes"
    var sizeDropdown = document.getElementById('filter-size-dropdown');
    if (sizeDropdown) {
      var sizeLabel = sizeDropdown.querySelector('.yarz-custom-dropdown__label');
  ...
  function clearFilters() {
    ...
    // Reset category custom dropdown
    var catDropdown = document.getElementById('filter-category-dropdown');
    if (catDropdown) {
  ...
  function openProduct(name) {
    ...
    if (state.stockBar && product.inStock) {
      ...
      if (totalStock > 0) {
        ...
        var bgColor = totalStock <= 5 ? 'rgba(220,38,38,0.08)' : totalStock <= 10 ? 'rgba(217,119,6,0.08)' : 'rgba(22,163,74,0.06)';
        var stockLabel = isOneSz ? totalStock + ' pieces in stock' : 'Select size to check stock';
        _stockBarHtml = '<div id="stock-urgency-bar" style="display:inline-flex;align-items:center;gap:6px;margin-bottom:16px;padding:5px 12px;background:' + bgColor + ';border-radius:6px;">' +
          '<span style="width:7px;height:7px;border-radius:50%;background:' + dotColor + ';flex-shrink:0;"></span>' +
          '<span id="stock-urgency-text" style="font-size:12px;font-weight:600;color:var(--text-primary,#1A1816);letter-spacing:0.01em;">' + stockLabel + '</span>' +
        '</div>';
      }
    }
  ...
  function renderHeroBannersFromStore(store) {
    ...
    for (var i = 1; i <= 5; i++) {
      ...
      if (store[imgKey]) {
        banners.push({
          image: store[imgKey],
          title: (store[titleKey] || '').trim().replace(/YARZ[\s\u2014\u2013\u2012\-]+Premium\s*Men[\u0027\u2019\u2018]?\s*s\s*Fashion/gi, '').trim(),
          link: store[linkKey] || '',
  ...
  return {
    ...
    _selectDropdownItem: _selectDropdownItem,
