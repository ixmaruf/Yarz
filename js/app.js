      ...
      try {
        ...
        if (cachedProducts) {
          ...
          if (Array.isArray(parsedProducts) && parsedProducts.length > 0) {
            ...
            if (cachedStoreInfo) {
              try { 
                state.storeInfo = JSON.parse(cachedStoreInfo); 
            ...
            if (hash.indexOf('#product/') === 0) {
              ...
              var idx = parseInt(hash.replace('#collection/', ''), 10);
              if (!isNaN(idx)) {
                setTimeout(function() { openCollection(idx, true); }, 50);
              }
            } else if (hash.indexOf('#category/') === 0) {
              var parts = hash.replace('#category/', '').split('/');
      ...
      if (cacheKey.indexOf('action=products') > -1 && data.success && data.products) {
        ...
        if (state.currentView === 'home') {
          ...
          if (data.storeInfo) {
            ...
            // Update cache
            try { localStorage.setItem('yarz_storeinfo_cache', JSON.stringify(state.storeInfo)); } catch(e) {}
          }
